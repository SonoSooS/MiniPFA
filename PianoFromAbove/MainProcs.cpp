    ...
    switch( msg )
    {
        case WM_COMMAND:
        {
            ...
            switch ( iId )
            {
                ...
                {
                    CheckActivity( TRUE );
                    GameState::State ePlayMode = GameState::Practice;
...
VOID SizeWindows( int iMainWidth, int iMainHeight )
{
    ...
    if ( hdwp ) hdwp = DeferWindowPos( hdwp, g_hWndGfx, NULL, 0, iBarHeight,  iMainWidth, iMainHeight - iBarHeight, swpFlags );
...
BOOL PlayFile( const wstring &sFile, int ePlayMode, bool bCustomSettings )
{
    ...
    if ( ePlayMode != GameState::Practice )
    {
